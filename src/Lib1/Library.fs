...
module Basic =
    ...
    let fib n =
        let rec _fib p1 p2 cnt =
            if cnt = 1 then p2 else _fib p2 (p1 + p2) (cnt - 1)
...
module Matrix =
    ...
    let sqrMat mat = product2_2 mat mat

    let toBinary n =
        let rec _f st n =
            if n = 1 then 1 :: st else _f (n % 2 :: st) (n / 2)
    ...
    let formArrayOfMatrices n =
        let mutable arr: (int array2d) array =
    ...
    let fib n =
        ...
        let _folder st mat incl = if incl then product2_2 st mat else st
